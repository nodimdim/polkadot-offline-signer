<template>
  <div>
    ...
    <b-button type="is-primary" expanded @click="openNewConnectionFrom"
      >Add new connection</b-button
    >
      ...
      <form @submit.prevent="addNewConnection">
        <div class="modal-card" style="width: auto">
          ...
          <footer class="modal-card-foot">
            <b-button label="Close" @click="closeNewConnectionFrom" />
            <b-button label="Create" native-type="submit" class="is-primary" />
          </footer>
        </div>
      </form>
    </b-modal>
...
export default {
  ...
  methods: {
    ...
    async addNewConnection() {
      ...
      try {
        const genesisHash = await rpcToNode(
          this.formFields.url,
          'chain_getBlockHash',
          [0]
        )
        const metadata = await rpcToNode(
          this.formFields.url,
          'state_getMetadata'
        )
